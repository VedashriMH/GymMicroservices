...
public class GymService {
    ...
    public ResponseDto getGym(int id) {
        ...
        List<ProgramDto> ProgramDtoList = webClientBuilder.baseUrl("http://localhost:8082")
                .build()
                .get()
        ...
        for (ProgramDto programDto : ProgramDtoList) {
            List<CustomerDto> customerDtoList = webClientBuilder.baseUrl("http://localhost:8761")
                    .build()
                    .get()
                    .uri("/cutsomer/" + programDto.getId())
                    .retrieve()
